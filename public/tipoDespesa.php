...
switch ($action) {
    case 'create':
        ...
        $despesa->idUsuario = 1; // Substitua pelo ID do usuário logado
    ...
    default:
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            echo "<tr><td>" . htmlspecialchars($row["nome"]) . "</td><td><a href='#' class='edit' data-id='" . $row["id"] . "' data-name='" . htmlspecialchars($row["nome"]) . "' data-idCentroCusto='" . $row["idCentroCusto"] . "'>Editar</a> <a href='#' class='delete' data-id='" . $row["id"] . "'>Excluir</a></td></tr>";
        }
